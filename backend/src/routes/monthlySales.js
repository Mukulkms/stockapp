...
// hua ho (chahe sales entry ho ya na ho) + jin mahino ki sales manually save ki
// gayi hai, sabko merge karke ek list banata hai (latest month pehle).
router.get('/:groupId', asyncHandler(async (req, res) => {
  ...
  const ensure = (y, m) => {
    ...
    if (!byMonth[k]) {
      byMonth[k] = { year: y, month: m, purchaseTotal: 0, salesAmount: 0, hasSalesEntry: false }
    }
    return byMonth[k]
  }
  ...
  const months = Object.values(byMonth)
    .map(r => {
      ...
      r.netProfit = +(r.salesAmount - r.purchaseTotal).toFixed(2)
      return r
    })
